...
export async function GET(request: Request) {
    // 1. Authorization Check (Optional but recommended for Cron)
    const authHeader = request.headers.get('authorization');
    if (process.env.CRON_SECRET && authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
        // Allow running without auth in development if needed, or strictly enforce
        // For now, if CRON_SECRET is set, we require it.
