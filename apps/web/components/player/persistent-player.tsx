...
export function PersistentPlayer() {
    const { isPlaying, togglePlay, currentTrack, isLoading, currentTime, duration, seekTo, volume, setVolume } = useAudio();
    ...
    return (
        <div className={cn(
            ...
            isExpanded ? "bottom-0 top-0 md:top-auto h-screen md:h-96" : "bottom-0 h-20 md:h-24"
        )}>
            ...
            >
                {isExpanded ? <ChevronDown className="w-6 h-6" /> : <ChevronUp className="w-6 h-6 animate-bounce" />}
            </button>
            ...
            <div className={cn(
                ...
                <div className={cn(
                    ...
                    <div className={cn(
                        ...
                        isExpanded ? "h-64 w-64 md:h-48 md:w-48" : "h-12 w-12 md:h-16 md:w-16"
                    )}>
                    ...
                    <div className={cn(
                        ...
                        <div className="flex items-center gap-2">
                            <span className={cn(
                                "truncate font-black font-display tracking-tight text-white uppercase italic transition-all",
                                isExpanded ? "text-2xl md:text-3xl" : "text-[13px] md:text-sm"
                            )}>
                                {title}
                            </span>
                            {isLive && !isExpanded && (
                ...
                <div className={cn(
                    ...
                    <div className="flex items-center gap-6 md:gap-8">
                        {isExpanded && (
                            <button className="text-zinc-500 hover:text-white" onClick={() => seekTo(Math.max(0, currentTime - 10))}>
                        ...
                        >
                            {isLoading ? (
                                ...
                                <Pause className={cn("fill-current", isExpanded ? "h-8 w-8" : "h-6 w-6")} />
                            ) : (
                                <Play className={cn("fill-current ml-1", isExpanded ? "h-8 w-8" : "h-6 w-6")} />
                            )}
                        </Button>
                        {isExpanded && (
                            <button className="text-zinc-500 hover:text-white" onClick={() => seekTo(Math.min(duration, currentTime + 10))}>
