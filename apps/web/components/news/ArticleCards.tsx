...
export function BreakingCard({ article, className }: CardProps) {
  return (
    <Link
      ...
      className={cn(
        "group relative block overflow-hidden rounded-3xl bg-zinc-900 aspect-[16/10]",
        className,
      )}
    >
      <Image
      ...
      <div className="absolute bottom-0 p-6 text-white">
        ...
        <p className="mt-2 text-sm text-zinc-300 line-clamp-1">
          {article.summary}
        </p>
      </div>
    </Link>
...
export function TrendingCard({ article, className }: CardProps) {
  return (
    <Link
      href={`/news/${article.slug}`}
      className={cn(
        "group flex flex-col gap-4 p-4 rounded-3xl bg-white border border-zinc-100 hover:border-primary/20 hover:shadow-xl transition-all duration-300",
        className,
      )}
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between mt-2 pt-4 border-t border-zinc-50">
          ...
          <span className="text-[10px] text-zinc-400 font-medium uppercase">
            {formatDisplayTime(article.published_at ?? article.created_at)}
          </span>
        </div>
      </div>
    </Link>
...
export function UpdateCard({ article, className }: CardProps) {
  return (
    <Link
      href={`/news/${article.slug}`}
      className={cn(
        "group flex gap-4 p-3 rounded-2xl hover:bg-zinc-50 transition-colors",
        className,
      )}
    >
...
export function ArchiveCard({ article, className }: CardProps) {
  return (
    <Link
      href={`/news/${article.slug}`}
      className={cn(
        "group flex items-center justify-between py-4 border-b border-zinc-100 hover:px-2 transition-all",
        className,
      )}
    >
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] px-1.5 py-0.5 bg-zinc-100 text-zinc-500 rounded uppercase font-bold">
            {article.category}
          </span>
          <span className="text-[10px] text-zinc-400">
            {formatDisplayTime(article.published_at ?? article.created_at)}
...
export function AudioCard({ article, className }: CardProps) {
  ...
  return (
    <div
      className={cn(
        "p-6 rounded-3xl bg-zinc-900 text-white relative overflow-hidden",
        className,
      )}
    >
      ...
      <div className="relative z-10 flex flex-col gap-4">
        <div className="flex items-center gap-2">
          <div className="px-2 py-0.5 bg-primary text-[10px] font-bold rounded-full uppercase">
            Audio Exclusive
          </div>
