...
export default function supabaseLoader({ src, width, quality }) {
  const supabaseUrl =
    ...
    "https://eybfcekeksdcnimfkgkc.supabase.co";
  ...
  if (src.includes(supabaseUrl)) {
    path = src.includes("/public/")
      ? src.split("/public/")[1]
      : src.replace(supabaseUrl, "");
  ...
  if (path && path.startsWith("http")) {
    ...
    if (path.includes("images.unsplash.com")) {
      try {
        ...
        urlObj.searchParams.set("q", String(quality ?? DEFAULT_IMAGE_QUALITY));
        urlObj.searchParams.set("auto", "format");
        return urlObj.toString();
      } catch {
        return `${path}${path.includes("?") ? "&" : "?"}w=${width}&q=${quality ?? DEFAULT_IMAGE_QUALITY}&auto=format`;
      }
    }
  ...
  return `${supabaseUrl}/storage/v1/render/image/public/${path}?width=${width}&quality=${quality ?? DEFAULT_IMAGE_QUALITY}&resize=contain`;
}
